...
def get_args_parser():
    ...
    parser.add_argument('--accum_iter', default=8, type=int, help='Accumulate gradient iterations (for increasing the effective batch size under memory constraints)')

    # Model parameters
    parser.add_argument('--llama_model_path', default='./pretrained/llama/', type=str, help='path to LLaMA pretrained checkpoint')
    parser.add_argument('--llama_type', default='7B', type=str, metavar='MODEL', help='Type of LLaMA model')
    ...
    parser.add_argument('--inference_model', type=str, default='best', help='inference model')
...
class STAR(Dataset):
    ...
    def _get_video(self, video_id, start, end):
        ...
        if len(video) > self.max_feats:
            ...
            video_len = len(video)
            # logging.warning(f'video_id: {video_id} video_len: {video_len} < max_feats: {self.max_feats}')
...
def LLaMA_VQA(args, **kwargs):
    with open(f'{args.llama_model_path}{args.llama_type}/params.json', "r") as f:
        params = json.loads(f.read())
    tokenizer = Tokenizer(model_path=f'{args.llama_model_path}/tokenizer.model')
    logging.info(f"Using model: {args.llama_type}")
    
    checkpoints = (Path(args.llama_model_path) / args.llama_type).glob("*.pth")
...
def inference(args):
    ...
    data_loader = load_data(args, tokenizer, split=args.split)

    model = LLaMA_VQA(args).to(torch.device(args.device))

    checkpoint = torch.load(f'./{args.inference_model}', map_location='cpu')
    _ = model.load_state_dict(checkpoint['model'], strict=False)

    submission_dict = {"Interaction": [], "Sequence": [], "Prediction": [], "Feasibility": []}

    question_type_map = {1: "Interaction", 2: "Sequence", 3: "Prediction", 4: "Feasibility"}
    ...
    for data in tqdm(data_loader):
        ...
        count = (logits != 0).sum(-1)
        prediction = (logits.sum(-1) / count).argmin(-1).item()
        
        if args.split == 'val':
            submission_dict[question_type_map[question_type]].append({
    ...
    with open('./results/submission.json', 'w') as outfile:
        json.dump(submission_dict, outfile, indent=4)
        
    logging.info("Submission saved to ./results/submission.json")
